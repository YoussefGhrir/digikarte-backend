...
@Table(
        name = "menus",
        indexes = {
                @Index(name = "idx_menus_slug", columnList = "slug"),
                @Index(name = "idx_menus_organization_id", columnList = "organization_id")
        }
)
@Getter
@Setter
...
public class Menu {
    ...
    @OnDelete(action = OnDeleteAction.CASCADE) // supprime les menus si l'organisation est supprimée
